...
class CustomDatasetPose(Dataset): # used to load and preprocess data
    ...
    def depth_to_pointcloud(self, masked_depth):
        ...
        u_valid = torch.tensor(u[valid_mask]).to(self.device)
        v_valid = torch.tensor(v[valid_mask]).to(self.device)
        z_valid = torch.tensor(masked_depth[valid_mask] / 1000.0).to(self.device)
    ...
    def load_6d_pose(self, folder_id: int = None, sample_id: int = None):
        ...
        obj_id = np.array(pose['obj_id'], dtype=np.uint8) # [1] ---> label
